...
let dbFolder = path.join(import.meta.dirname, '..', 'data');
let dbConfig = JSON.parse(fs.readFileSync(path.join(dbFolder, 'database-in-use.json'), 'utf-8'));
let databaseToUse = path.join(dbFolder, dbConfig.file);
...
if (fs.existsSync(databaseToUse)) {
  ...
  console.log("✅ Ansluten till databas:", databaseToUse);
} else {
  console.log("❌ Databasfil hittades inte:", databaseToUse);
}
